...
// returns bigNumber
function getAddressBalance(address, cb) {
  Address.findOne({ a_id: address }, function(err, addressDoc) {
    if (err || !addressDoc || !addressDoc.balance) {
      return cb(err, new BigNumber(0));
    }

    var addressBalanceBN = new BigNumber(addressDoc.balance);
    
    return cb(null, addressBalanceBN);
  });
}
...
module.exports = {
  ...
  balance_supply: function(cb) {
    Address.find({is_crowdsale: false}, 'balance').where('balance').gt(0).exec(function(err, docs) {
      var countBN = new BigNumber(0);
      
      module.exports.syncLoop(docs.length, function (loop) {
        var i = loop.iteration();

        var balanceString = docs[i].balance.toString();
        countBN = countBN.plus(balanceString);
  ...
  get_balance_supply: function(cb) {

    if (settings.supply === 'BALANCES') {

      module.exports.balance_supply(function(supplyBN) {
        getAddressBalance(settings.exclude_address, function(err, addressBalanceBN) {
          if (err) {
            return cb(err, supplyBN.div(100000000).toString());
          }

          var resultSupply = supplyBN.minus(addressBalanceBN).div(100000000).toString();

          return cb(null, resultSupply);
        });
  ...
  get_supply: function(cb) {
    if ( settings.supply == 'HEAVY' ) {
      ...
      module.exports.balance_supply(function(supplyBN) {
        var supplyString = supplyBN.div(100000000).toString();
        return cb(supplyString);
      });
