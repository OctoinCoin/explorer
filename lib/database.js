...
var  lib = require('./explorer');
var  settings = require('./settings');
var  poloniex = require('./markets/poloniex');
var  bittrex = require('./markets/bittrex');
var  bleutrade = require('./markets/bleutrade');
var  cryptsy = require('./markets/cryptsy');
var  cryptopia = require('./markets/cryptopia');
var  yobit = require('./markets/yobit');
var  empoex = require('./markets/empoex');
var  ccex = require('./markets/ccex');
var coinmarketcapService = require('./markets/coinmarketcap');
//var  BTC38 = require('./markets/BTC38');
...
function update_address(hash, txid, amount, type, cb) {
  ...
  var is_crowdsale = settings.exclude_txs.indexOf(txid) !== -1;
    ...
    if (address) {
      ...
      if ( hash == 'coinbase' ) {
        ...
        lib.is_unique(address.txs, txid, function(unique, index) {
          ...
          if (unique == true) {
            tx_array.push({is_crowdsale: is_crowdsale, addresses: txid, type: type});
      ...
      if (type == 'vin') {
        var newAddress = new Address({
          a_id: hash,
          txs: [ {is_crowdsale: is_crowdsale, addresses: txid, type: 'vin'} ],
          sent: amount,
          balance: amount,
          is_crowdsale: is_crowdsale
        });
      } else {
        var newAddress = new Address({
          a_id: hash,
          txs: [ {is_crowdsale: is_crowdsale, addresses: txid, type: 'vout'} ],
          received: amount,
          balance: amount,
          is_crowdsale: is_crowdsale
        });
                ...
                lib.syncLoop(vout.length, function (subloop) {
                  ...
                  lib.calculate_total(vout, function(total){
                    var newTx = new Tx({
                      ...
                      is_crowdsale: settings.exclude_txs.indexOf(txid) !== -1
                    });
...
function get_market_data(market, cb) {
  switch(market) {
    ...
    case 'empoex':
      ...
      break;
    case 'coinmarketcap':
      coinmarketcapService.getCoinInfo(settings.markets.coin, function(err, obj) {
        return cb(err, obj);
      });
      break;
    default:
...
module.exports = {
  ...
  update_richlist: function(list, cb){
    if(list == 'received') {
      Address.find({is_crowdsale: false}).sort({received: 'desc'}).limit(100).exec(function(err, addresses){
      ...
      Address.find({is_crowdsale: false}).sort({balance: 'desc'}).limit(100).exec(function(err, addresses){
        Richlist.update({coin: settings.coin}, {
          balance: addresses,
        }, function() {
          return cb();
  ...
  get_last_txs: function(count, min, cb) {
    Tx.find({is_crowdsale: false, 'total': {$gt: min}}).sort({_id: 'desc'}).limit(count).exec(function(err, txs){
      if (err) {
        return cb(err);
      } else {
        return cb(txs);
